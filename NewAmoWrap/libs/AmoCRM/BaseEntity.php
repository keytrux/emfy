...
use CURLFile;
use DateTime;
use AmoCRM_Wrap\Helpers\Config;
use AmoCRM_Wrap\Helpers\CustomField;
use AmoCRM_Wrap\Helpers\Value;
use Exception;
use stdClass;
...
abstract class BaseEntity extends Base
{
        ...
        try {
            ...
            if (!empty($data->id)) {
                ...
                if (!empty($data->id)) {
                    ...
                    $res = AmoCRM::cUrl("api/v4/{$typeUrl}/{$data->id}/links");
                    $this->links = $res->_embedded->links;
                }
                $contactsLink = $this->getLinksByType('contacts');
                foreach ($contactsLink as $contact) {
                    $this->contactsId[] = $contact->to_entity_id;
                }
                $companiesLink = $this->getLinksByType('companies');
                foreach ($companiesLink as $comapny) {
                    $this->companyId = $comapny->to_entity_id;
                    break;
                }
                $leadsLink = $this->getLinksByType('leads');
                foreach ($leadsLink as $lead) {
                    $this->leadsId[] = $lead->to_entity_id;
                }

                $this->type = isset($data->note_type) ? (int)$data->note_type : null;

                if (isset($data->_embedded->tags) && is_array($data->_embedded->tags)) {
                ...
                if (isset($data->custom_fields_values) && is_array($data->custom_fields_values)) {
                    foreach ($data->custom_fields_values as $custom_field) {
    ...
    public function addCustomField($name, $type = CustomField::TYPE_TEXT, $enums = array())
    {
        ...
        $data['request']['fields']['add'] = array(
            array(
                ...
                'origin' => 'Roistat AmoCRM Wrap',
                'disabled' => false,
            )
        );
        $res = AmoCRM::cUrl('private/api/v4/json/fields/set', $data);
    ...
    {
        $this->elementId = intval(Base::onlyNumbers($elementId));
    ...
    protected function load($id) {
        $typeUrl = $this->config['url'];
        $link = "api/v4/$typeUrl/$id";

        $res = AmoCRM::cUrl($link);

        // Улучшенная проверка ответа
        if ($res === null) {
            throw new AmoWrapException("API вернул пустой ответ для $typeUrl ID: $id");
        }

        if (!isset($res->id)) {
            // Для контактов/сделок API v4 возвращает объект напрямую, а не в _embedded
            if (isset($res->id)) {
                $this->loadInRaw($res);
                return;
            }
            throw new AmoWrapException("Неверный формат ответа API для $typeUrl ID: $id");
        }
    ...
    private function saveBase($data = array())
    {
        ...
        $requestData = array($this->getRawBase($data));
        $res = AmoCRM::cUrl("api/v4/{$this->config['url']}", $requestData, null, false, $method);

        if ($res !== null && isset($res->_embedded->{$this->config['url']}[0]->id)) {
            $this->setLinks($requestData[0], $res->_embedded->{$this->config['url']}[0]->id);
            ...
            $this->load($res->_embedded->{$this->config['url']}[0]->id);
    ...
    private function getRawBase($data = array())
    {
        ...
        if ($this->responsibleUserId !== null) {
            $data['responsible_user_id'] = intval($this->responsibleUserId);
        }
        ...
        $data['_embedded']['tags'] = $this->tags;//implode(',', $this->tags);
        ...
        if (count($customFields) > 0) {
            foreach ($customFields as $customFieldObj) {
                ...
                foreach ($customFieldObj->getValues() as $valueObj) {
                    $value = array(
                        //TODO:: fix enum custom fields, this one is not acceptable by api
                        //'enums' => $valueObj->getEnum(),
                        'value' => $valueObj->getValue(),
                    );
                    $values[] = $value;
                }
                $data['custom_fields_values'][] = array(
                    'field_id' => intval($customFieldObj->getId()),
                    'values' => $values
                );
            }
        }
     ...
     * @param int $entityId
     */
    private function setLinks($data, $entityId) {
        $leadLinks = $this->getLinksByType('leads');
        $contactLinks = $this->getLinksByType('contacts');
        $companyLinks = $this->getLinksByType('companies');

        // linking and unlinking contacts
        $contactsLinksId = $this->getLinkEntitiesId($contactLinks);
        $toUnlink = array_filter($contactsLinksId, function ($v, $k) use ($data) {
            if (!in_array($v, $data['contacts_id'])){
                return intval($v);
            }
        }, ARRAY_FILTER_USE_BOTH);
        $toLink = array_filter($data['contacts_id'], function ($v, $k) use ($contactsLinksId) {
            if (!in_array($v, $contactsLinksId)){
                return intval($v);
            }
        }, ARRAY_FILTER_USE_BOTH);

        $linkRequestData = array();
        foreach ($toLink as $newLink) {
            $linkRequestData[] = array(
                'to_entity_id' => intval($newLink),
                'to_entity_type' => 'contacts'
            );
        }
        if (!empty($linkRequestData)) {
            $linkResponse = Base::cUrl("/api/v4/{$this->config['url']}/{$entityId}/link", $linkRequestData, null, false, 'POST');
        }

        $unlinkRequestData = array();
        foreach ($toUnlink as $oldLink) {
            $unlinkRequestData[] = array(
                'to_entity_id' => intval($oldLink),
                'to_entity_type' => 'contacts'
            );
        }
        if (!empty($unlinkRequestData)) {
            $unlinkResponse = Base::cUrl("/api/v4/{$this->config['url']}/{$entityId}/unlink", $unlinkRequestData, null, false, 'POST');
        }
        // linking and unlinking leads
        $leadsLinksId = $this->getLinkEntitiesId($leadLinks);
        $toUnlink = array_filter($leadsLinksId, function ($v, $k) use ($data) {
            if (!in_array($v, $data['leads_id'], true)){
                return (int)$v;
            }
        }, ARRAY_FILTER_USE_BOTH);

        if(!empty($data['leads_id'])) {
            $toLink = array_filter($data['leads_id'], function ($v, $k) use ($leadsLinksId) {
                if (!in_array($v, $leadsLinksId, true)){
                    return (int) $v;
                }
            }, ARRAY_FILTER_USE_BOTH);
        ...
        // linking and unlinking company
        $companyLinkId = $this->companyId;
        //TODO:: compete linking company

        return null;
    }

    private function getLinkEntitiesId($entities){
        $result = array();
        if ($entities) {
            $result = array_map(function ($v) {
                return $v->to_entity_id;
            }, $entities);
        }
        return $result;
    }
     ...
     * @return array
     */
    protected function getLinksByType($type) {
        return array_filter($this->links, function ($v ,$k) use ($type) {
            return $v->to_entity_type == $type;
        }, ARRAY_FILTER_USE_BOTH);
    }
